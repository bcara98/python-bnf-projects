...
#Description: This program performs the Needleman–Wunsch global sequence alignment for 2 input fasta files and prints out the alignment with an alignment score
...
#helper function to calculate the score at a specific (x,y) coordinates on the grid and the direction its based on (Diaglonal, Left, Up)
def get_cordinate_value(x,y,grid,seq1,seq2):
    ...
    vals = [] #stores all the calculated values when adding either a match, mismatch, or gap with either of the previous grid cells
    ...
    for i in range(0,len(coordinates)):
        if coordinates[i][0] >= 0 and coordinates[i][1] >= 0:
            ...
            continue
    #if no previous cells had valid coordinates (origin with 0,0) then return a list with the default value of 0 and no direction 
    if len(vals) == 0:
        return [0,'']
    else:
        #else find the maximum score and return that along withe the direction it came from
        max_score = [0,'']
        for i in range(0,len(vals)):
            if i == 0:
                max_score[0] = vals[i][0]
                max_score[1] = vals[i][1]
            elif i != 0 and max_score[0] < vals[i][0]:
                max_score[0] = vals[i][0]
                max_score[1] = vals[i][1]
        return max_score
...
def get_alignment(grid,seq1,seq2):
    #start scanning the cell at the bottom most right corner, score is the value of that cell
    y_cor = len(grid)-1
    x_cor= len(grid[len(grid)-1])-1
    score = grid[y_cor][x_cor][0]
    alignment = []

    #while loop runs until you get to the origin of the grid (0,0) coordinates
    end = False
    while end == False:
        ...
        if grid[y_cor][x_cor][1] == 'D':
            ...
            #break loop if origin is reached (direction of previous cell direction is empty)
            end = True
...
align = get_alignment(grid,seq1,seq2) #traceback the grid to get the alignment
...
#print_grid_space(grid, seq1, seq2)
